...
description: Use when building GENUINELY DISPOSABLE work - throwaway or time-boxed prototypes, quick experiments, hackathons, spikes, demos, proof-of-concepts, take-homes, interviews - including making progress on a spec bigger than the time available, where prioritization and a coherent result matter more than finishing. NOT for small real changes on maintained projects (those keep worktree+PR — see the "small real change" tier in global CLAUDE.md). Symptoms - "spin up a project fast", "quick prototype", "try this idea", "quick demo", "just get something working".
---
...
**When NOT to use — use the full heavy ticket workflow instead:**
...
**Graduating to real** — when the ask becomes real auth, a real database, or deploying to prod infra (GitHub, a cloud host, etc.), the prototype is leaving throwaway territory. Say so: the deliberate stubs (in-memory store, no-auth) were the point, and going real changes the demo (e.g. instant → a login step). Switch out of fast mode for the real parts, and treat external systems with care (next section).

## Permission to Go Fast

This skill **overrides** the heavy defaults in your global instructions (and any superpowers/brainstorming hook) for the duration of the prototype. Without this section, the agent will keep reaching for brainstorming, TDD, and worktrees out of habit and burn the time box.
...
| Brainstorming skill (`superpowers:brainstorming` — Q&A, 2-3 approaches, design doc) | 2-minute scope-and-cut, below. |
| Plan-review ceremonies | Skip. |
| Git worktree (`superpowers:using-git-worktrees`) | Work directly on `main` in a fresh dir. |
...
| Code review + CI + PR + merge (`superpowers:requesting-code-review`) | Skip. Commit to `main` as save-points only. |
...
Use the project's chosen stack and the scaffold/install/dev commands appropriate to it — **do not assume a package manager or framework.** If the project has a `.claude/CLAUDE.md`, use the commands defined there (Verification Commands section) for build/dev/test/lint. Confirm the empty app loads via the project's dev command BEFORE going further.

```bash
# Generic shape (adapt entirely to the chosen stack & package manager):
#   <scaffold command for the framework>   # create the empty app
#   <install command>                      # install deps
#   <dev command>                          # confirm it loads BEFORE going further
```

Verify scaffolding flags against the installed version — don't trust memory for scaffolding CLIs. **If the scaffold pins a major version newer than your training (check the lockfile/manifest such as `package.json`/`pyproject.toml`, and watch for an `AGENTS.md`/`CLAUDE.md` warning), read the framework's local/bundled docs — or use the `verify-library-api` skill — BEFORE writing any data/routing code.** Version traps (e.g. changed async signatures, renamed config) cost more debugging time mid-build than the 60 seconds it takes to read the doc. For a DB-backed slice, get one connection-proving read working before building the feature.

Before building, also check the project's `.claude/CLAUDE.md` and any `~/.claude/projects/<this-project-encoded-cwd>/memory/*.md` for "Sharp Edges"; skip if none exist.
...
When several cut-list features are independent, don't build them one at a time — **fan out a wave of subagents, one feature each, then review the wave together.** This is the highest-leverage way to use AI tooling on a prototype, and the review step is where you keep ownership of the output. (Plain parallel `Task` subagents cover this; team tools like `TeamCreate` vary across Claude Code versions — confirm what exists in the live session before relying on them.)
...
- **Problem:** Habit (or a brainstorming/superpowers hook) pulls you into brainstorming/TDD/worktrees and the clock runs out.
...
- **Problem:** Scaffold pins a major version with breaking changes (changed async signatures, renamed config); you write the old pattern and burn time debugging.
- **Fix:** Check the lockfile/manifest (`package.json`/`pyproject.toml`/etc.) and any `AGENTS.md`/`CLAUDE.md`. Read the framework's local/bundled docs (or use `verify-library-api`) before data/routing code.
...
- **Fix:** Put the LLM call behind ONE function with a deterministic fallback so the app runs with no key and swapping provider/model is one line. Keep the key server-side. The provider can change on a whim ("use this provider, I have credits") — isolation makes that free.
...
- **Fix:** Before ANY write to a real external system (DB migration, deploy), **verify the target is the throwaway/sandbox, not production** — check the actual project ref/URL the tool resolves to, don't assume. If it points at prod, stop and surface it. External logins (cloud hosts, GitHub, auth providers) and their secrets are the user's to do — hand those off with exact commands; never fake or guess them.

## Freshness check (verify anchors are fresh)

Before relying on a file, symbol, or external resource you remember from earlier, confirm it still exists — assumptions go stale fast in a moving prototype:

- **File exists:** check the path is still present before importing/editing it.
- **Symbol grep:** `rg`/`grep` for the function/type/route name before calling it, rather than trusting memory.
- **External anchors:** run the Post-Merge / Post-Setup Verification commands defined in the ticket (or the project's `.claude/CLAUDE.md` Verification Commands) to confirm the backing resource is real. *(Example: if the project uses Supabase, query `information_schema`; adapt to the project's stack — Prisma schema file, a migrations dir, an ORM model module, etc.)*
